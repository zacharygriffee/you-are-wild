        ...
        const App = {
            ...
            _refreshMerchantStock(merchant, force = false) {
                ...
                if (force || needsStock || stale) {
                    merchant.stock = this._defaultMerchantStock();
            ...
            showTrade(targetId) {
                ...
                merchant.stock.forEach((item, index) => {
                    ...
                    html += `<div class="option-card" style="text-align:left;cursor:default;"><div style="display:flex;justify-content:space-between;gap:8px;"><div><div style="font-weight:700;color:var(--text-primary)">${def.icon || '?'} ${item.name}</div><div style="font-size:11px;color:var(--text-muted)">${def.desc || ''}</div></div><div style="font-size:12px;color:var(--text-muted)">Qty ${item.qty} | ${item.price}g</div></div><button class="nav-btn" style="margin-top:8px;padding:4px 8px;font-size:11px" ${disabled} onclick="App.buyFromMerchant('${this._unitKey(merchant)}',${index})">Buy</button></div>`;
                });
                ...
                if (this.inventory.length === 0) {
                    ...
                    this.inventory.forEach(item => {
                        ...
                        html += `<div class="option-card" style="text-align:left;cursor:default;"><div style="display:flex;justify-content:space-between;gap:8px;"><div><div style="font-weight:700;color:var(--text-primary)">${def.icon || '?'} ${item.name}</div><div style="font-size:11px;color:var(--text-muted)">${def.desc || ''}</div></div><div style="font-size:12px;color:var(--text-muted)">${price}g</div></div><button class="nav-btn" style="margin-top:8px;padding:4px 8px;font-size:11px" onclick="App.sellToMerchant('${this._unitKey(merchant)}','${String(item.id).replace(/'/g, "\\'")}')">Sell</button></div>`;
                    });
                }
            ...
            renderUnitCard(unit, index, type) {
                ...
                if (isParty && !this.combatState.active) {
                    ...
                    actionButtons += `<button class="action-btn" onclick="event.stopPropagation();App.showPartyMemberStats(${index})">Stats</button>`;
                    if (!isLeader) actionButtons += `<button class="action-btn" onclick="event.stopPropagation();App.setPartyLeader(${index})">Lead</button>`;
                    ...
                    if (isAlly) {
                        ...
                        actionButtons += `<button class="action-btn" style="color:var(--accent-danger)" onclick="event.stopPropagation();App.dismissPartyMember(${index})">Dismiss</button>`;
                    }
                    actionButtons += `</div>`;
                }
                if (!isParty && isCorpse) {
                ...
                if (!isParty && unit.CPun > 0 && !isCorpse) {
                    ...
                    if (this.targetSelection) {
                        ...
                        if (unit.quest) {
                            actionButtons += `<button class="action-btn primary" onclick="event.stopPropagation();App.acceptQuestFromUnit('${targetKey}')">📜 ${unit.questAccepted ? 'View Quest' : 'Accept Quest'}</button>`;
                        }
                        if (unit.disposition === this.DISPOSITION.MERCHANT) {
                            actionButtons += `<button class="action-btn primary" onclick="event.stopPropagation();App.showTrade('${targetKey}')">🪙 Trade</button>`;
                        }
                        actionButtons += `</div>`;
                    }
                }
