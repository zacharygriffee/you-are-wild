...
const MODULE_MARKETPLACE = {
    ...
    ui: {
            ...
            const html = `
                <div style="max-width: 1000px; margin: 0 auto; padding: 24px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                        <div>
                            <h1 style="color: var(--accent-primary); margin: 0;">🏪 Module Marketplace</h1>
                            <p style="color: var(--text-muted); margin: 8px 0 0 0;">
                                Discover community-made biomes, species, and features
                            </p>
                        </div>
                        <div style="display: flex; gap: 8px;">
                            <button class="nav-btn" onclick="MODULE_MARKETPLACE.ui.showInstalled()">
                                📦 My Modules
                            </button>
                            <button class="nav-btn" onclick="MODULE_MARKETPLACE.ui.showCreate()">
                                ✏️ Create
                            </button>
                        </div>
                    </div>
                    ...
                    <div style="background: var(--bg-secondary); border: 1px solid var(--border-default); 
                        ...
                        <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                            ...
                            <select onchange="MODULE_MARKETPLACE.ui.filterByType(this.value)"
                                    style="background: var(--bg-tertiary); border: 1px solid var(--border-default); 
                                           border-radius: var(--radius-sm); padding: 8px 12px; color: var(--text-primary); font-size: 14px;">
                                <option value="all">All Types</option>
                                <option value="biome_pack">Biomes</option>
                                <option value="species_pack">Species</option>
                                <option value="feature_pack">Features</option>
                                <option value="content_pack">Content</option>
                            </select>
                            <select onchange="MODULE_MARKETPLACE.ui.sort(this.value)"
                                    style="background: var(--bg-tertiary); border: 1px solid var(--border-default); 
                                           border-radius: var(--radius-sm); padding: 8px 12px; color: var(--text-primary); font-size: 14px;">
                    ...
                    <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid var(--border-default);">
                        ...
                        <div style="background: linear-gradient(135deg, var(--bg-secondary), var(--bg-tertiary)); 
                            ...
                            <div style="flex: 1;">
                                <h3 style="color: var(--accent-primary); margin: 0 0 8px 0;">Desert of Whispers</h3>
                                <p style="color: var(--text-secondary); margin: 0 0 12px 0;">
                                    Our most popular biome expansion! Navigate scorching sands, discover hidden oases, 
                                    and survive encounters with the legendary sand-wyrms.
                                </p>
                                <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                                    <span style="background: var(--bg-tertiary); padding: 4px 8px; border-radius: var(--radius-sm); 
                                                 font-size: 12px; color: var(--text-muted);">15k+ downloads</span>
                                    <span style="background: var(--bg-tertiary); padding: 4px 8px; border-radius: var(--radius-sm); 
                                                 font-size: 12px; color: var(--text-muted);">⭐ 4.8 rating</span>
                                </div>
                            </div>
                ...
                <div style="background: var(--bg-secondary); border: 1px solid var(--border-default); 
                    ...
                    <div style="padding: 16px;">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                            <h3 style="margin: 0; font-size: 16px; color: var(--text-primary);">${mod.name}</h3>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div style="display: flex; gap: 12px; font-size: 12px; color: var(--text-muted);">
                                <span>⬇️ ${this.formatNumber(mod.downloads)}</span>
                                <span>⭐ ${mod.rating}</span>
                            </div>
                            <button class="nav-btn" style="padding: 6px 12px; font-size: 12px;"
                                    onclick="event.stopPropagation(); MODULE_MARKETPLACE.ui.install('${mod.id}')">
                                Install
                            </button>
                        </div>
                        <div style="margin-top: 8px; font-size: 11px; color: var(--text-muted);">
                            by ${mod.author} • v${mod.version} • ${mod.size}
                        </div>
                    </div>
                </div>
            `;
        ...
        async install(moduleId) {
            const mod = this.featuredModules.find(m => m.id === moduleId);
            if (!mod) return;
            
            // Simulate download
            App.log.push({ text: `Downloading ${mod.name}...`, type: 'discovery' });
            App.renderLog();
            ...
            App.log.push({ text: `Installed and enabled ${mod.name}!`, type: 'discovery' });
            App.renderLog();
