        ...
        const App = {
            ...
            showMobilePartyContext(index) {
                ...
                const roleOptions = Object.entries(this.PARTY_ROLES).map(([key, label]) => `<option value="${key}" ${role === key ? 'selected' : ''}>${label}</option>`).join('');
                const orderOptions = Object.entries(this.PARTY_AI_ORDERS).map(([key, label]) => `<option value="${key}" ${order === key ? 'selected' : ''}>${label}</option>`).join('');
                ...
                html += actionButton('Stats', 'stats');
                if (unit !== this.player && !unit.mc) {
                    if (this._getPartyLeader() !== unit) html += actionButton('Make Leader', 'lead');
                    html += `<label class="mobile-context-field" onclick="event.stopPropagation()"><span>Role</span><select class="nav-btn" aria-label="Party role for ${unitLabel}" onchange="event.stopPropagation();App.setPartyRole(${index},this.value)">${roleOptions}</select></label>`;
                    html += `<label class="mobile-context-field" onclick="event.stopPropagation()"><span>AI Order</span><select class="nav-btn" aria-label="AI order for ${unitLabel}" onchange="event.stopPropagation();App.setPartyAIOrder(${index},this.value)">${orderOptions}</select></label>`;
                    html += actionButton('Dismiss', 'dismiss', ' danger');
                }
