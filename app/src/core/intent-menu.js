...
const YAW_INTENT_MENU = {
    ...
    openSubActionSheet(app, type, targetRef, action, source = 'sheet', anchorEvent = null) {
        ...
        subActions.filter(sub => sub.id !== defaultSub).forEach(sub => {
            ...
            html += `<button class="action-btn" role="menuitem" data-command-mode="exploration" data-command-intent="${app._escapeHtml(`${action}:${sub.id}`)}" data-command-grammar="actor-target-intent" title="${label}${app._escapeHtml(settingHint)}" aria-label="${label}${app._escapeHtml(settingHint)}"${disabled} onclick="App.selectIntent('${type}',${targetArg},'${action}','${commandSource}','${String(sub.id).replace(/'/g, "\\'")}')">${sub.icon || ''} ${label}</button>`;
        });
