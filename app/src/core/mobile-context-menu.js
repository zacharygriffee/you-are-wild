...
const YAW_MOBILE_CONTEXT_MENU = {
  showParty(app, index) {
    ...
    const actionButton = (label, action, extraClass = '') => `<button class="action-btn${extraClass}" role="menuitem" title="${app._escapeHtml(label)}" aria-label="${app._escapeHtml(label)}" onclick="App.mobilePartyContextAction('${action}',${index})">${app._escapeHtml(label)}</button>`;
    ...
    let html = `<div class="mobile-context-menu" id="mobile-context-menu" role="dialog" aria-modal="true" aria-label="${app._escapeHtml(menuLabel)}" aria-labelledby="mobile-context-menu-title"><div class="mobile-context-menu-title" id="mobile-context-menu-title">${unit.icon || ''} ${unitLabel}</div><div class="mobile-context-menu-actions" role="menu">`;
    ...
    if (unit !== app.player && !unit.mc) {
      ...
      html += `<label class="mobile-context-field" onclick="event.stopPropagation()"><span>${app._escapeHtml(roleLabel)}</span><select class="nav-btn" aria-label="${app._escapeHtml(roleAria)}" title="${app._escapeHtml(roleDescription)}" onchange="event.stopPropagation();App.mobilePartyContextSetRole(${index},this.value)">${roleOptions}</select><small>${app._escapeHtml(roleDescription)}</small></label>`;
      html += `<label class="mobile-context-field" onclick="event.stopPropagation()"><span>${app._escapeHtml(orderLabel)}</span><select class="nav-btn" aria-label="${app._escapeHtml(orderAria)}" title="${app._escapeHtml(orderDescription)}" onchange="event.stopPropagation();App.mobilePartyContextSetAIOrder(${index},this.value)">${orderOptions}</select><small>${app._escapeHtml(orderDescription)}</small></label>`;
