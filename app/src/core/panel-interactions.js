...
const YAW_PANEL_INTERACTIONS = {
    ...
    feed(app, actor, label) {
        ...
        const buttons = (app.feedSelection.subIds || []).map(subId => {
            ...
            return `<button class="action-btn" data-command-intent="${intent}" title="${subLabel}" aria-label="${subLabel}" onclick="App._executeFeedSubAction('${safeSubId}', App.activeActor || App._currentCombatActor() || App.player)">${icon} ${subLabel}</button>`;
        }).join('');
        return `<div class="panel-interaction-tray combat-feed-tray" role="region" aria-label="${title || label}"><div class="target-action-row" data-command-surface="feed-options" aria-label="${title || label}">${buttons}<button class="action-btn" data-command-control="cancel-feed" title="${clearLabel}" aria-label="${clearLabel}" onclick="App.cancelTargetSelection()">${clearLabel}</button></div></div>`;
    },
    ...
    sync(app, actor, label) {
        ...
        return `<div class="panel-interaction-tray combat-sync-tray" role="status" aria-label="${trayLabel}"><div class="target-action-row" data-command-surface="${surface}" aria-label="${trayLabel}">${controls}</div></div>`;
    }
};
