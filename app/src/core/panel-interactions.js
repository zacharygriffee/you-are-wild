...
const YAW_PANEL_INTERACTIONS = {
    ...
    feed(app, actor, label) {
        ...
        return `<div class="panel-interaction-tray combat-feed-tray" data-command-surface="feed-options" data-command-mode="combat" data-command-grammar="actor-target-intent" role="region" aria-label="${title || label}"><div class="target-action-row" data-command-surface="feed-options" data-command-mode="combat" data-command-grammar="actor-target-intent" aria-label="${title || label}">${buttons}<button class="action-btn" data-command-surface="feed-options" data-command-mode="combat" data-command-control="cancel-feed" data-command-slot="exit" title="${clearLabel}" aria-label="${clearLabel}" onclick="App.cancelTargetSelection()">${clearLabel}</button></div></div>`;
    },
    ...
    sync(app, actor, label) {
        ...
        if (app.syncSelection.phase === 'choose') {
            ...
            return `<div class="panel-interaction-tray combat-sync-tray" data-command-surface="sync-intents" data-command-mode="combat" data-command-grammar="actor-target-intent" role="region" aria-label="${title || label}"><div class="target-action-row" data-command-surface="sync-intents" data-command-mode="combat" data-command-grammar="actor-target-intent" aria-label="${title || label}">${syncButton('sync_fight', '⚔️', 'combat.sync.action.fight', 'Group Fight')}${syncButton('sync_flirt', '😘', 'combat.sync.action.flirt', 'Group Talk')}${syncButton('sync_fuck', '🔥', 'combat.sync.action.fuck', 'Group Play')}${syncButton('sync_feed', '🍽️', 'combat.sync.action.feed', 'Group Feed')}<button class="action-btn" data-command-surface="sync-intents" data-command-mode="combat" data-command-control="cancel-sync" data-command-slot="exit" title="${clearLabel}" aria-label="${clearLabel}" onclick="App.cancelTargetSelection()">${clearLabel}</button></div></div>`;
        }
        ...
        let controls = `<button class="action-btn" data-command-surface="${surface}" data-command-mode="combat" data-command-control="cancel-sync" data-command-slot="exit" title="${clearLabel}" aria-label="${clearLabel}" onclick="App.cancelTargetSelection()">${clearLabel}</button>`;
