...
const YAW_STATS_PANEL = {
    ...
    showPartyMember(app, index) {
        ...
        const html = `<div class="party-stats-view" role="region" aria-label="${app._escapeHtml(app._label('party.statsFor', 'Show stats for {name}', { name: unit.name }))}">
    ...
    showPerkSelection(app) {
        ...
        filters.forEach(([value, label]) => {
            ...
            html += `<button class="action-chip${active}" role="tab" aria-selected="${app.perkTreeFilter === value ? 'true' : 'false'}" data-perk-filter="${escapedValue}" title="${filterLabel}" aria-label="${filterLabel}" onclick="App.setPerkTreeFilter('${escapedValue}')">${filterLabel}</button>`;
        });
