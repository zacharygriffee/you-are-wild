...
test('Defeat return-to-menu uses in-app confirmation', () => {
  ...
  cancelled.App.showScreen = screen => { cancelledScreen = screen; };
  cancelled.App.updateLanguage('es');
  cancelled.App.combatState.active = true;
  cancelled.App.endCombat('defeat');
  assertEqual(cancelled.confirmations.length, 0, 'Defeat return should not call the browser-native confirm dialog');
  assert(cancelled.App.pendingConfirm, 'Defeat return should open an in-app confirmation dialog');
  assertEqual(cancelled.App.pendingConfirm.message, 'Derrota! Volver al menu?', 'Defeat return confirmation should localize');
  assertEqual(cancelled.App.pendingConfirm.confirmLabel, 'Volver al menu', 'Defeat return confirm button should localize');
  assertContains(cancelled.body.innerHTML, 'role="dialog"', 'Defeat return confirmation should render an in-app modal dialog');
  cancelled.App.resolveConfirmDialog(false);
  assertEqual(cancelledScreen, null, 'Cancelling defeat return should leave the current app screen alone');
  assertEqual(cancelled.App.pendingConfirm, null, 'Cancelling defeat return should clear pending confirmation state');

  const approved = loadAppForCombat(() => 0.5, { confirm: true });
  let approvedScreen = null;
  approved.App.showScreen = screen => { approvedScreen = screen; };
  approved.App.combatState.active = true;
  approved.App.endCombat('defeat');
  assertEqual(approved.confirmations.length, 0, 'Approved defeat return should not call the browser-native confirm dialog');
  assert(approved.App.pendingConfirm, 'Approved defeat return should open an in-app confirmation dialog');
