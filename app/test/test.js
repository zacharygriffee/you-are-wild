...
test('Defeat recovery helper module is registered before app code', () => {
  ...
  assert(buildContent.indexOf("'src/core/defeat-recovery.js'") < buildContent.indexOf("'src/core/save-load-flow.js'"), 'Defeat recovery should load before save-load flow');
...
test('Panel command helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/panel-commands.js'", 'Panel command helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/panel-interactions.js'") < buildContent.indexOf("'src/core/panel-commands.js'"), 'Panel commands should load after panel interaction tray helpers');
  assert(buildContent.indexOf("'src/core/panel-commands.js'") < buildContent.indexOf("'src/core/app.js'"), 'Panel command helper should load before app.js');
  assertContains(panelCommandsContent, 'const YAW_PANEL_COMMANDS = {', 'Panel command helper should expose the command service');
  ...
  assertContains(appContent, 'YAW_PANEL_COMMANDS.outsideAction(this, action, type, index)', 'App outside action wrapper should delegate to the helper');
...
test('Unit stats helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/unit-stats.js'", 'Unit stats helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/unit-stats.js'") < buildContent.indexOf("'src/core/unit-card-status.js'"), 'Unit stats helper should load before unit card status');
  assert(buildContent.indexOf("'src/core/unit-stats.js'") < buildContent.indexOf("'src/core/stats-panel.js'"), 'Unit stats helper should load before stats panel rendering');
  assert(buildContent.indexOf("'src/core/unit-stats.js'") < buildContent.indexOf("'src/core/app.js'"), 'Unit stats helper should load before app.js');
  assertContains(unitStatsContent, 'const YAW_UNIT_STATS = {', 'Unit stats helper should expose the unit stats service');
  assertContains(unitStatsContent, 'coreStats(unit)', 'Unit stats helper should own core stat normalization');
  assertContains(unitStatsContent, 'displayStats(app, unit)', 'Unit stats helper should own display stat normalization');
  assertContains(unitStatsContent, "app._getSpeciesBaseStats(source.species || 'human')", 'Display stats should use species base fallbacks');
  assertContains(appContent, 'YAW_UNIT_STATS.coreStats(unit)', 'App core stats wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_UNIT_STATS.displayStats(this, unit)', 'App display stats wrapper should delegate to the helper');
});

test('Marked target action helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/marked-target-actions.js'", 'Marked target action helper should be included in SCRIPT_ORDER');
...
test('Recruitment flow helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/recruitment-flow.js'", 'Recruitment flow helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/recruitment-flow.js'") < buildContent.indexOf("'src/core/unit-card.js'"), 'Recruitment helper should load before unit card rendering');
  assert(buildContent.indexOf("'src/core/recruitment-flow.js'") < buildContent.indexOf("'src/core/app.js'"), 'Recruitment helper should load before app.js');
  assertContains(recruitmentFlowContent, 'const YAW_RECRUITMENT_FLOW = {', 'Recruitment helper should expose the recruitment service');
  assertContains(recruitmentFlowContent, 'score(app, actor, target)', 'Recruitment helper should own recruitment scoring');
  assertContains(recruitmentFlowContent, "app._hasBaselineInteractionEligibility(target, 'recruit')", 'Recruitment helper should preserve baseline eligibility gating');
  assertContains(recruitmentFlowContent, 'recruit(app, target, actor = app.player, options = {})', 'Recruitment helper should own party transfer behavior');
  assertContains(appContent, 'YAW_RECRUITMENT_FLOW.score(this, actor, target)', 'App recruit score wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_RECRUITMENT_FLOW.canRecruit(this, actor, target)', 'App recruit gate wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_RECRUITMENT_FLOW.recruit(this, target, actor, options)', 'App recruit wrapper should delegate to the helper');
});
...
test('Interaction state helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/interaction-state.js'", 'Interaction state helper should be included in SCRIPT_ORDER');
...
test('Action UI helper module is registered before app code', () => {
  ...
  assertContains(actionRulesContent, 'explorationActionRating(app, entry, actor, target = null, purpose =', 'Action rules helper should own exploration ratings');
  assertContains(actionRulesContent, 'targetDodgeRoll(app, actor, target, action =', 'Action rules helper should own target dodge rolls');
  assertNotContains(actionRulesContent, 'Math.random', 'Action rules helper should not use ambient randomness');
...
test('Unit container helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/unit-containers.js'", 'Unit container helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/unit-lifecycle.js'") < buildContent.indexOf("'src/core/unit-containers.js'"), 'Unit containers should load after unit lifecycle');
  assert(buildContent.indexOf("'src/core/unit-containers.js'") < buildContent.indexOf("'src/core/app.js'"), 'Unit container helper should load before app.js');
  assertContains(unitContainersContent, 'const YAW_UNIT_CONTAINERS = {', 'Unit container helper should expose the container service');
  assertContains(unitContainersContent, 'capacity(unit, container =', 'Unit container helper should own capacity math');
  assertContains(unitContainersContent, 'contents(unit, container =', 'Unit container helper should own container access');
  assertContains(unitContainersContent, 'canFit(predator, prey, container =', 'Unit container helper should own fit checks');
  assertContains(unitContainersContent, 'failureMessage(app, actor, target, container =', 'Unit container helper should own localized capacity failure text');
  assertContains(unitContainersContent, 'summary(app, unit, container =', 'Unit container helper should own capacity summaries');
...
test('Combat action helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/combat-state-roll.js'", 'Combat state roll helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/combat-state-roll.js'") < buildContent.indexOf("'src/core/combat-rules.js'"), 'Combat state roll helper should load before combat rules');
  assert(buildContent.indexOf("'src/core/combat-state-roll.js'") < buildContent.indexOf("'src/core/combat-status.js'"), 'Combat state roll helper should load before combat status');
  assert(buildContent.indexOf("'src/core/combat-state-roll.js'") < buildContent.indexOf("'src/core/app.js'"), 'Combat state roll helper should load before app.js');
  assertContains(combatStateRollContent, 'const YAW_COMBAT_STATE_ROLL = {', 'Combat state roll helper should expose the roll service');
  assertContains(combatStateRollContent, "roll(app, namespace, unit = null, purpose = 'roll')", 'Combat state roll helper should own combat roll seed construction');
  assertContains(combatStateRollContent, 'app._worldRoll(', 'Combat state roll helper should route through world-seeded rolls');
  assertContains(combatStateRollContent, 'app.combatState.round || 0', 'Combat state roll helper should include combat round in the seed');
  assertContains(combatStateRollContent, 'app.combatState.currentTurn || 0', 'Combat state roll helper should include current turn in the seed');
  assertNotContains(combatStateRollContent, 'Math.random', 'Combat state roll helper should not use ambient randomness');
  assertContains(appContent, 'YAW_COMBAT_STATE_ROLL.roll(this, namespace, unit, purpose)', 'App combat state roll wrapper should delegate to the helper');
  assertContains(buildContent, "'src/core/combat-rules.js'", 'Combat rules helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/combat-rules.js'") < buildContent.indexOf("'src/core/combat-actions.js'"), 'Combat rules should load before combat action helpers');
  assert(buildContent.indexOf("'src/core/combat-rules.js'") < buildContent.indexOf("'src/core/combat-targeting.js'"), 'Combat rules should load before combat targeting helpers');
  ...
  assert(buildContent.indexOf("'src/core/combat-status.js'") < buildContent.indexOf("'src/core/app.js'"), 'Combat status should load before app.js');
...
test('Combat ally helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/combat-allies.js'", 'Combat ally helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/combat-allies.js'") < buildContent.indexOf("'src/core/app.js'"), 'Combat ally helper should load before app.js');
  assertContains(combatAlliesContent, 'const YAW_COMBAT_ALLIES = {', 'Combat ally helper should expose the ally service');
  assertContains(combatAlliesContent, 'healWounded(app, ally)', 'Combat ally helper should own wounded ally support behavior');
  ...
  assertContains(appContent, 'YAW_COMBAT_ALLIES.runPostCombatScavengers(this)', 'App scavenger wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_COMBAT_ALLIES.takeTurn(this, ally)', 'App ally turn wrapper should delegate to the helper');
});

test('Combat enemy helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/combat-enemies.js'", 'Combat enemy helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/combat-enemies.js'") < buildContent.indexOf("'src/core/app.js'"), 'Combat enemy helper should load before app.js');
  assertContains(combatEnemiesContent, 'const YAW_COMBAT_ENEMIES = {', 'Combat enemy helper should expose the enemy service');
  assertContains(combatEnemiesContent, 'shouldFlee(app, enemy, targets)', 'Combat enemy helper should own morale flee decisions');
  assertContains(combatEnemiesContent, 'callReinforcement(app, enemy)', 'Combat enemy helper should own reinforcement calls');
  assertContains(combatEnemiesContent, 'selectTarget(app, enemy, targets)', 'Combat enemy helper should own enemy target selection');
  assertContains(combatEnemiesContent, 'takeTurn(app, enemy)', 'Combat enemy helper should own enemy turn AI');
  assertContains(combatEnemiesContent, "app._combatStateRoll('combat-enemy-flee'", 'Combat enemy helper should preserve deterministic morale rolls');
  assertContains(combatEnemiesContent, "app._combatStateRoll('combat-reinforcement'", 'Combat enemy helper should preserve deterministic reinforcement rolls');
  assertContains(combatEnemiesContent, "app._combatStateRoll('combat-target-tasty'", 'Combat enemy helper should preserve deterministic target tie-breaks');
  assertNotContains(combatEnemiesContent, 'Math.random', 'Combat enemy helper should not use ambient randomness');
  assertNotContains(combatEnemiesContent, 'Date.now', 'Combat enemy helper should not use wall-clock reinforcement IDs');
  assertContains(appContent, 'YAW_COMBAT_ENEMIES.shouldFlee(this, enemy, targets)', 'App enemy flee wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_COMBAT_ENEMIES.callReinforcement(this, enemy)', 'App reinforcement wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_COMBAT_ENEMIES.selectTarget(this, enemy, targets)', 'App enemy target wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_COMBAT_ENEMIES.takeTurn(this, enemy)', 'App enemy turn wrapper should delegate to the helper');
});

test('Combat sync helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/combat-sync.js'", 'Combat sync helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/combat-sync.js'") < buildContent.indexOf("'src/core/app.js'"), 'Combat sync helper should load before app.js');
...
test('Combat intent helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/combat-intents.js'", 'Combat intent helper should be included in SCRIPT_ORDER');
...
test('Combat actor state helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/combat-actor-state.js'", 'Combat actor state helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/mobile-combat-toolbelt.js'") < buildContent.indexOf("'src/core/combat-actor-state.js'"), 'Combat actor state should load after the mobile combat toolbelt it coordinates with');
  ...
  assertContains(combatActorStateContent, 'turnOrderBadge(app, unit)', 'Combat actor state helper should own turn order badge rendering');
  assertContains(combatActorStateContent, 'statusText(app, unit)', 'Combat actor state helper should own combat actor status text');
  ...
  assertContains(appContent, 'YAW_COMBAT_ACTOR_STATE.turnOrderBadge(this, unit)', 'App turn badge wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_COMBAT_ACTOR_STATE.statusText(this, unit)', 'App combat status text wrapper should delegate to the helper');
...
test('Unit card helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/unit-card.js'", 'Unit card helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/unit-card.js'") < buildContent.indexOf("'src/core/app.js'"), 'Unit card helper should load before app.js');
...
test('Equipment system helper module is registered before app code', () => {
  ...
  assertContains(equipmentSystemContent, 'const YAW_EQUIPMENT_SYSTEM = {', 'Equipment system helper should expose the equipment service');
...
test('Perk flow helper module is registered before app code', () => {
  ...
  assertContains(perkFlowContent, 'completeRespec(app)', 'Perk flow helper should own perk respec behavior');
  assertContains(perkFlowContent, 'app.showConfirmDialog({', 'Perk respec should keep in-app confirmation');
  assertContains(perkFlowContent, 'app.autoSave()', 'Perk changes should keep autosaving');
  assertNotContains(perkFlowContent, "document.getElementById('scene-description')", 'Perk helper should not render into center tile content');
  assertContains(appContent, 'YAW_PERK_FLOW.availableChoices(this, unit)', 'App perk choices wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_PERK_FLOW.choose(this, perkId)', 'App choose perk wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_PERK_FLOW.respec(this, skipConfirm)', 'App respec wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_PERK_FLOW.debugGrant(this, count)', 'App debug grant wrapper should delegate to the helper');
});

test('Stats panel helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/stats-panel.js'", 'Stats panel helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/stats-panel.js'") < buildContent.indexOf("'src/core/app.js'"), 'Stats panel helper should load before app.js');
  assertContains(statsPanelContent, 'const YAW_STATS_PANEL = {', 'Stats panel helper should expose the stats panel service');
  assertContains(statsPanelContent, 'showPartyMember(app, index)', 'Stats panel helper should own party member stats rendering');
...
test('Quest panel helper module is registered before app code', () => {
  assertContains(buildContent, "'src/core/quest-panel.js'", 'Quest panel helper should be included in SCRIPT_ORDER');
  assert(buildContent.indexOf("'src/core/quest-panel.js'") < buildContent.indexOf("'src/core/app.js'"), 'Quest panel helper should load before app.js');
  assertContains(questPanelContent, 'const YAW_QUEST_PANEL = {', 'Quest panel helper should expose the quest panel service');
...
test('Mobile unit strip helper module is registered before app code', () => {
  ...
  assert(buildContent.indexOf("'src/core/mobile-combat-toolbelt.js'") < buildContent.indexOf("'src/core/mobile-unit-strips.js'"), 'Mobile unit strip helper should load after the mobile combat toolbelt helper');
...
test('Mobile context menu helper module is registered before app code', () => {
  ...
  assertContains(mobileContextMenuContent, 'const YAW_MOBILE_CONTEXT_MENU = {', 'Mobile context menu helper should expose the context menu service');
  assertContains(appContent, 'YAW_MOBILE_CONTEXT_MENU.showParty(this, index)', 'App mobile party context should delegate to the helper');
  assertContains(appContent, 'YAW_MOBILE_CONTEXT_MENU.partyAction(this, action, index)', 'App mobile party context actions should delegate to the helper');
  assertContains(appContent, 'YAW_MOBILE_CONTEXT_MENU.setPartyRole(this, index, role)', 'App mobile party role changes should delegate to the helper');
  assertContains(appContent, 'YAW_MOBILE_CONTEXT_MENU.setPartyAIOrder(this, index, order)', 'App mobile party AI order changes should delegate to the helper');
  assertContains(appContent, 'YAW_MOBILE_CONTEXT_MENU.showCreature(this, targetId)', 'App mobile creature context should delegate to the helper');
...
test('Save slot flow helper module is registered before app code', () => {
  ...
  assertContains(saveSlotFlowContent, 'const YAW_SAVE_SLOT_FLOW = {', 'Save slot flow helper should expose the slot flow service');
  assertContains(saveSlotFlowContent, "['load', 'save', 'new'].includes(mode)", 'Save slot flow should clamp manager modes');
  assertContains(saveSlotFlowContent, "app._normalizeSaveSlotName(slotName, null)", 'Destructive new/delete slot flows should reject malformed slot names');
  assertContains(saveSlotFlowContent, "app._label('save.confirm.newGameOverwrite'", 'New game overwrite warning should remain localized');
  ...
  assertContains(appContent, 'YAW_SAVE_SLOT_FLOW.saveToSlot(this, slotName)', 'App manual save wrapper should delegate to the helper');
...
test('Structure navigation helper module is registered before app code', () => {
  ...
  assertContains(structureNavigationContent, 'const YAW_STRUCTURE_NAVIGATION = {', 'Structure navigation helper should expose the traversal service');
  assertContains(structureNavigationContent, 'currentExplorationTile(app)', 'Structure navigation helper should own current exploration tile resolution');
  assertContains(structureNavigationContent, 'enter(app)', 'Structure navigation helper should own entering structures');
  assertContains(structureNavigationContent, 'exit(app)', 'Structure navigation helper should own exiting structures');
  assertContains(structureNavigationContent, 'moveInterior(app, dx, dy)', 'Structure navigation helper should own interior movement');
  assertContains(structureNavigationContent, 'rest(app)', 'Structure navigation helper should own safe rest flow');
  assertContains(appContent, 'YAW_STRUCTURE_NAVIGATION.currentExplorationTile(this)', 'App current tile wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_STRUCTURE_NAVIGATION.enter(this)', 'App enter-structure wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_STRUCTURE_NAVIGATION.exit(this)', 'App exit-structure wrapper should delegate to the helper');
  assertContains(appContent, 'YAW_STRUCTURE_NAVIGATION.moveInterior(this, dx, dy)', 'App interior movement wrapper should delegate to the helper');
...
test('Status effects present', () => {
  const combatStatusSources = `${appContent}\n${combatStatusContent}\n${combatAlliesContent}\n${combatEnemiesContent}`;
  assertContains(combatStatusSources, 'poisoned', 'poisoned status missing');
  assertContains(combatStatusSources, 'restrained', 'restrained status missing');
  assertContains(combatStatusSources, 'enveloped', 'enveloped status missing');
...
test('New game flow is slot-aware and warns before destructive slot changes', () => {
  ...
  assertContains(contentContent, "'save.saveDescription': 'Choose the slot that will continue this run. Auto-save will keep updating the saved slot; the previous active slot stops progressing.'", 'Save mode should explain active auto-save slot behavior');
  ...
  assertContains(contentContent, "'save.confirm.manualOverwrite': 'Overwrite {slot} with the current game and continue auto-saving there? This cannot be undone.'", 'Manual save should warn before overwriting another occupied slot and changing auto-save slot');
...
test('Sync failure and submissive recruit prompts localize', () => {
  ...
  assertEqual(recruitCase.App.pendingConfirm.message, 'Mouse esta listo para seguirte. Reclutarlo para tu grupo?', 'Recruitment confirmation should localize');
  ...
  assertEqual(syncRecruitCase.App.pendingConfirm.message, 'Mouse esta listo para seguirte. Reclutarlo para tu grupo?', 'Sync recruitment confirmation should localize');
});
...
test('Container capacity allows prey that fits', () => {
  ...
  const prey = makeUnit('Prey', { size: 2, CPun: 1, MPun: 100, Flee: 1 });
  const result = App._doSubAction('feast', 'swallow', predator, prey, 'Predator', 's');
  assertContains(result, 'belly', 'Allowed swallow should produce belly result text');
...
test('Expanded unit card shows containment capacity usage', () => {
  ...
  assertContains(html, 'Belly: 2/6', 'Unit card should show belly used/capacity');
  assertContains(html, 'Inner: 0/3', 'Unit card should show inner capacity');
  assertContains(html, 'Reserve: 0/3', 'Unit card should show reserve capacity');
});
...
test('Defeat ends combat into a durable recovery state', () => {
  ...
  const enemy = makeUnit('Plantfolk', { id: 'enemy-defeat-state', disposition: App.DISPOSITION.ENEMY, CPun: 20, MPun: 20 });
  App.player = player;
  App.party = [player];
  App.creatures = [enemy];
  ...
  App.combatState = {
    ...
    processing: false,
    xpEarned: 0,
    turnQueue: [{ unit: enemy, initiative: 10 }],
    syncActions: []
  };
  let autoSaveCalls = 0;
  ...
  assertEqual(App.combatState.active, false, 'Defeat should end active combat');
  assertEqual(App.defeatState.pending, true, 'Defeat should set a pending defeated state');
  assertEqual(App.defeatState.defeatedAt.x, 7, 'Defeat state should preserve defeated location');
...
test('Defeat regeneration restores the safe anchor without clearing defeated tile enemies', () => {
  ...
  App.worldMap = new Map([
    ...
    ['7,-2', { ...App.getBaseTile(7, -2), explored: true, biome: 'jungle', creatures: [enemy] }]
  ]);
  let autoSaveCalls = 0;
  App.autoSave = () => { autoSaveCalls += 1; };

  const regenerated = App.regenerateFromDefeat();

  assertEqual(regenerated, true, 'Regenerate should complete');
  assertEqual(App.defeatState, null, 'Regenerate should clear defeated state');
  assertEqual(App.location.x, 0, 'Regenerate should move to safe anchor x');
...
test('Safe place action is available only on safe overworld rest-capable structures', () => {
  ...
  App.creatures = [];
  App.inInterior = true;
  assertEqual(App._canSetSafeAnchor(), false, 'Interior rooms should not be safe anchors until interior persistence is explicit');

  App.inInterior = false;
  App.combatState.active = true;
  assertEqual(App._canSetSafeAnchor(), false, 'Combat should block home marking');
});
...
test('Defeated combat refresh snapshots are rejected and cleared', () => {
  ...
  App.party = [player];
  App.creatures = [enemy];
  App.location = { x: 6, y: 6 };
  App.worldMap = new Map([['6,6', { ...App.getBaseTile(6, 6), explored: true, biome: 'jungle', creatures: [enemy] }]]);
  App.activeSlot = 'slot1';
  App.combatState = {
    active: true,
    round: 3,
...
test('Defeat renders recovery choices instead of return confirmation', () => {
  ...
  defeated.App.showScreen = screen => { shownScreen = screen; };
  defeated.App.updateLanguage('es');
  defeated.App.player = makeUnit('You', { CPun: 0, knockedOut: true });
  defeated.App.party = [defeated.App.player];
  defeated.App.combatState.active = true;
  defeated.App.endCombat('defeat');
...
test('Fallback interact menu localizes labels and keeps target indexes stable', () => {
  ...
  assertContains(elements.get('enemies-content').innerHTML, "toggleExplorationTarget('creature','friendly-1')", 'Creature panel remains the target selection surface');
  assertNotContains(elements.get('enemies-content').innerHTML, "showIntentMenu('creature','friendly-1','desktop')", 'Creature panel should not duplicate marked-target actions behind a visible menu');
...
test('Self-included group social action shares pleasure across participants', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'share play with Target', 'Self-included social action should log shared semantics');
});
...
test('Self-included group social action shares pleasure with multiple helpers', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'Target, Helper A, Helper B share play with Target', 'Three-participant social action should log shared semantics');
});
...
test('Self-included group flirt shares pleasure with multiple helpers', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'Target, Helper A, Helper B share talk with Target', 'Three-participant flirt should log shared semantics');
});
...
test('Self-included group feast rejects instead of routing self-consumption', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'cannot eat themself', 'Self-included feast should explain the rule');
});
...
test('Single actor feast rejects direct self-targeting', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'cannot eat themself', 'Direct self-feast should explain the rule');
});
...
test('Group exploration outcome summaries localize', () => {
  ...
  assertContains(social.App.log[social.App.log.length - 1].text, 'comparten jugar con Target', 'Self-included group social summary should localize');
...
test('Single exploration action result logs localize', () => {
  ...
  assertContains(feast.App.log[feast.App.log.length - 1].text, 'Tu come a Prey', 'Single feast result should localize');
  assertContains(feast.App.log[feast.App.log.length - 1].text, 'barriga de Tu', 'Single feast owner label should localize');
...
test('Marked target action rejects stale selected actors without falling back to player', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'Select a living actor before using talk on marked targets.', 'Rejected stale actor command should explain the correction');
});
...
test('Capable actor can resolve one action across multiple exploration targets', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'Actor termina una accion multiobjetivo de hablar sobre Target A, Target B.', 'Successful multi-target action summary should localize');
});
...
test('Single-actor group wrapper preserves rejected action outcomes', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'cannot eat themself', 'Rejected self-feast should keep correction guidance');
...
test('Exploration cards expose multi-target selection and context actions', () => {
  const { App, elements } = loadAppForCombat(() => 0);
  const actor = makeUnit('Actor', { id: 'actor-1', Flir: 30, cha: 20 });
  const helper = makeUnit('Helper', { id: 'helper-1', Flir: 20, cha: 12 });
  ...
  App.player = actor;
  App.party = [actor, helper, allyTarget];
  App.creatures = [creatureTarget];
  ...
  const allyTargetCard = App.renderUnitCard(allyTarget, 2, 'party');
  const creatureTargetCard = App.renderUnitCard(creatureTarget, 0, 'creature');
  assertContains(partyHtml, 'aria-label="Asignar Actor como actor"', 'Party actor button should label actor role selection instead of an action command');
  ...
  assertContains(mobileActorChip, 'aria-label="Asignar Actor como actor"', 'Mobile actor button should label actor role selection instead of an action command');
...
test('Marked self-included group target resolves through shared group semantics', () => {
  ...
  assertContains(App.log[App.log.length - 1].text, 'share talk with Target', 'Marked self-included action should route to group semantics');
});
...
test('Tile event feed renders safe actor intent and semantic metadata', () => {
  ...
  assertEqual(event.actorName, 'Scout <One>', 'Tile event should preserve actor metadata');
  assertEqual(event.intent, 'Scout', 'Tile event should preserve intent metadata');
...
test('Desktop party card management labels localize', () => {
  ...
  App.renderParty();
  let html = elements.get('party-content').innerHTML;
  assertContains(html, 'aria-label="Asignar Ally B como actor"', 'Actor selection control should expose localized accessible role label');
...
test('Desktop creature card status and detail labels localize', () => {
  ...
  assertContains(panelHtml, 'Barriga: 2/8', 'Capacity summary should localize container labels');
...
test('Player card and character stats use the same live display stats', () => {
  ...
  assertContains(cardHtml, 'aria-label="Spirit: 42%"', 'Player card pleasure bar should use live top-level vitals');
...
test('Unit cards and mobile chips render compact tactical bars accessibly', () => {
  ...
  assertContains(partyCard, 'aria-label="Spirit: 25%"', 'Party card should expose pleasure bar label');
...
test('Unit selection controls distinguish focus actor target and combat pick semantics', () => {
  ...
  assertContains(creatureCard, 'data-action-scope="creature-selection-utility" aria-label="Target and utility controls for Guide"', 'Desktop creature action row should identify target/utility scope');
  ...
  assertContains(mobileCreatureChip, 'data-action-scope="creature-selection-utility" aria-label="Target and utility controls for Guide"', 'Mobile creature action row should identify target/utility scope');
...
test('Save slot destructive confirmations localize', async () => {
  ...
  assertEqual(manualSave.App.pendingConfirm.message, 'Sobrescribir Slot 3 con la partida actual y continuar autoguardando ahi? Esta accion no se puede deshacer.', 'Manual overwrite warning should use active locale and display slot label');
...
test('Save slot status feedback uses display slot labels', async () => {
  ...
  assertEqual(saved.App.saveManagerStatus.message, 'Partida guardada en Slot 2. El autoguardado ahora actualiza este slot.', 'Save success status should use localized display slot label');
  assertContains(saved.elements.get('save-manager').innerHTML, 'Partida guardada en Slot 2. El autoguardado ahora actualiza este slot.', 'Save manager should show the successful save status');
  ...
  assertEqual(recovered, true, 'Fallen legacy save should continue loading');
...
test('Mobile unit chip actions expose localized accessible labels', () => {
  ...
  assertContains(partyHtml, 'aria-label="Asignar Ally como actor"', 'Mobile party actor button should localize accessible role label');
