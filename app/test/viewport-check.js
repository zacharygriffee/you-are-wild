...
async function checkViewport(browser, name, width, height) {
  ...
  if (width < 600) {
    ...
    assert(mobileControls.miniMapHeight >= 170, `${name}: mobile traversal grid should keep a usable minimum height`);
    assert(mobileControls.centerTileWidth >= 150, `${name}: mobile current tile should keep a broad target column for stage presence`);
    assert(mobileControls.centerTileHeight >= 112, `${name}: mobile current tile should leave room for larger presence controls`);
    ...
    assert(mobileControls.minCenterPresenceWidth >= 52 && mobileControls.minCenterPresenceHeight >= 52, `${name}: mobile current tile presence badges should keep roomy finger-sized tap targets`);
