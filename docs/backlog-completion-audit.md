...
| Delivery | Full build, viewport, interaction, branding, artifact, and diff gates | 0.18.2 adds isolated public Alpha missions and exact-fixture browser-agent coverage while retaining the 0.18.1 release gates. Repeat the same gates for every patch. |
...
| Delivery mutations require explicit authority | `docs/next-objectives.md` autonomous boundary and Sites workflow | Local verification commands below; no test or build command stages, commits, pushes, versions, or deploys | Proved locally; future commit/push/version/Sites changes remain operator-gated |
