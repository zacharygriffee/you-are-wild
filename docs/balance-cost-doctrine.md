...
Multi-target Fight and explicit Chew are opt-in distributed effects rather than repeated full-strength attacks. With `N` targets, a novice begins at `1 / N` of their ordinary contribution per target. Each action owns separate persistent practice that recovers the missing contribution continuously up to mastery, while explicit equipment, natural-technique, instinct, or true-area declarations may recover additional effect. In many-to-many actions, each participant is scaled from their own practice and technique before contributions are combined. Practice is awarded once per completed command, diminishes within the same encounter/tile-day context, persists per party member, and never directly raises general `Figh`, `Feas`, or global XP. Single-target actions, mutual sparring, paired commands, action costs, reach, status application, and every unprofiled intent retain their prior rules.
...
Action buttons should expose compact cost previews through `title` / `aria-label`. When a profiled multi-target Fight or Chew is composed, desktop and mobile surfaces also show the current per-target contribution percentage before commitment.
