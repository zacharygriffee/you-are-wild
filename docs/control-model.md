...
- `Feed` requires a semantic migration before the contextual picker is treated as complete. The proposed canonical direction is that the actor offers their whole self or an authored portion to the target consumer; support/healing, nursing, willingness, coercion, and target-forcing behavior must not remain conflated under ambiguous actor/target roles.
