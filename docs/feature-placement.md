...
| Canonical Feed / self-offering lifecycle | Interaction plans, consent/willingness, containment, creature capabilities, balance, Scene Feed | High | Tier-dependent | Feed sub-actions, containment profiles, content providers | Whether Feed becomes actor-to-target whole/portion offering; where Tend/Heal and Nurse move; which willingness and survival rules are baseline |
| Contextual action-variant orchestrator | Composer, action registry, reach/capacity previews, accessibility, group plans, mod APIs | Medium | Safe-capable with tier-filtered variants | `InteractionPlan.subAction` and the current sub-action registry | Picker timing, direct-dispatch rule, disabled-reason contract, mixed group validity, and bounded mod registration |
