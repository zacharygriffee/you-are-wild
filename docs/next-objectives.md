...
- Localization has a first-pass foundation: `CONTENT.locales` exposes English/Spanish keys, `CONTENT.t()` supports variable interpolation, `CONTENT.setLanguage()` persists language preferences, settings exposes an interface language selector, high-traffic action/target labels route through locale keys, and mobile party/creature context-menu labels plus accessible management field labels localize through the active language
...
- Continue auditing lower-traffic custom controls for keyboard focus order, visible focus, and complete `aria-label` coverage
