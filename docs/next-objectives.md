...
- Playwright viewport smoke checks now run through `npm run full-build` and can be run directly with `npm run test:viewport`; they verify mobile/desktop built-page load, horizontal overflow, save/new-game modal bounds, large-map localization hooks, and mobile party-panel button reachability after the slide transition settles
