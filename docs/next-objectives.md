...
- Keep primary intent and sub-action resolution separate. Radial entries should map to existing action keys and existing sub-action definitions, then route through the shared dispatcher using a normalized command shape such as actor id, action, sub-action, target id/type, and source. Use bottom/action sheets for sub-actions, confirmations, target prompts, modded action overflow, and keyboard/mouse accessibility.
