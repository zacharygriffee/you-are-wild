...
1. Continue manual longer-save playtests for Death/Defeat V3. Automated coverage now locks save/resume before and after companion settlement, repeated bags, full-pack partial retrieval without gold duplication, interior defeat placement, legacy state migration, and scoped Hardcore deletion. Remaining novel coverage should focus on future module/environment terminal causes and long-running sparse-save histories rather than duplicating these fixtures.
2. Continue manual Drop Off, Rejoin, and defeat-stranded companion playtests across overworld and interiors. Automated coverage now locks overworld save/load/rejoin and exact-room interior delta persistence. Follow-up work may add full interior-session reload coverage, roster maps, rescue/capture routes, or companion extraction, but must preserve persistent tile placement as the baseline.
...
5. Redesign the canonical `Feed` contract before extending its current mixed sub-actions. The intended baseline direction is actor-to-target offering: the selected actor voluntarily offers their whole self or an authored renewable portion for the selected target to consume. Livestock-like or explicitly willing actors may offer themselves; ordinary creatures do not consent by default. Slimes and other authored species may offer a nonterminal portion. Resolution must check actor capability and willingness, target appetite/container/capacity, size, reach, disposition, survival consequences, group distribution, and Scene Feed outcomes. Existing Tend/Heal, Nurse, Sacrifice/Offer, forced-feeding, slurp, and fragment behavior must be audited and either migrated, renamed, or placed under a clearer primary verb rather than silently changing saves or commands. Coercive or explicit framing remains an opt-in rated provider/variant; core state and default presentation stay neutral.
