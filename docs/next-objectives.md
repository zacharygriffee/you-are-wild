...
- **Build:** 162/162 tests pass, 10/10 lint modules clean, dist fresh
...
- Further harden party-to-party feeding when mixed self-target/helper/prey groups are selected; selected helpers now reject or route more clearly in the covered one-to-many/many-to-one cases, self-included `feed` tends the target instead of consuming helpers, and self-included social actions share pleasure with participants, but fight/feast and richer non-asymmetric groups still need explicit semantics
