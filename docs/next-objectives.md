...
- **Build:** 161/161 tests pass, 10/10 lint modules clean, dist fresh
...
- Further harden party-to-party feeding when mixed self-target/helper/prey groups are selected; selected helpers now reject or route more clearly in the covered one-to-many/many-to-one cases, and self-included `feed` tends the target instead of consuming helpers, but other self-included/non-asymmetric actions still need explicit semantics
