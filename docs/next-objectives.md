...
3. Maintain automated mobile regression coverage for blocked commands, clipping, overflow, touch targets, high contrast, reduced motion, and 12-20px font scaling without treating incremental patches as the future phone-flow redesign.
...
3. Build an interaction-wide balance matrix before the next tuning pass, then use `docs/balance-scenario-baseline.md` for comparisons. The matrix must cover Fight, Talk, Play, Feed, Feast, Flee, self actions, single-target, multi-target, group, success, and committed failure outcomes across condition, vitality, hunger, Spirit, cooldowns, XP, practice, time, and turn cost. In particular, feeding oneself or another creature must consume an appropriate resource from the source rather than create free recovery: depending on the authored Feed variant this may be condition, vitality/renewable body mass, nutrition reserve, hunger, or a cooldown. Do not choose one universal cost until the variant matrix establishes what the source is actually giving.
...
16. Design Mobile Interaction Flow V2 as a future UX workstream rather than an immediate layout patch. It must make party-versus-enemy context switching obvious and low-friction, keep contextual structure actions such as Enter and Search reachable when party or creature panels are open, preserve actor/target state where appropriate, and decide whether phone navigation is best served by tabs, a unified roster sheet, or another deliberate information architecture. Retain current accessibility and no-blocked-command regression gates until that design is selected.
